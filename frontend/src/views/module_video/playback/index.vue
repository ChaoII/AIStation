  ...
  <div class="pb-page">
    ...
    <header class="pb-topbar">
      ...
      <el-tooltip :content="devicePanelOpen ? '隐藏设备面板' : '显示设备面板'" placement="bottom">
        <button class="layout-btn" :class="{ active: devicePanelOpen }" @click="devicePanelOpen = !devicePanelOpen">
      ...
      <div class="topbar-center">
        <div class="search-group">
          <div class="field-row">
            <el-date-picker v-model="searchDate" type="date" placeholder="选择日期" size="small" value-format="YYYY-MM-DD" class="field-date" />
            <el-time-picker v-model="searchStartTime" placeholder="开始" size="small" value-format="HH:mm:ss" format="HH:mm" class="field-time" />
            <span class="time-sep">—</span>
            <el-time-picker v-model="searchEndTime" placeholder="结束" size="small" value-format="HH:mm:ss" format="HH:mm" class="field-time" />
    ...
    <div class="pb-body">
      ...
      <aside v-show="devicePanelOpen" class="live-device-panel">
        ...
        <div class="tree-filter">
          <el-input v-model="deviceSearch" size="small" placeholder="搜索设备..." clearable :prefix-icon="Search" />
        </div>
        <el-tree
          ref="deviceTreeRef"
          :data="deviceTreeData"
      ...
      <div class="pb-content">
        ...
        <div class="pb-player">
          ...
          <div v-else class="player-empty">
            <div class="empty-icon">
              <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" opacity="0.4"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
            </div>
          ...
          <div v-if="currentRecording" class="player-info">
            <span class="info-cam">{{ currentRecording.camera_name || `#${currentRecording.camera_id}` }}</span>
            <span class="info-sep" />
            <span class="info-time">{{ formatDateTime(currentRecording.start_time) }} — {{ formatDateTime(currentRecording.end_time) }}</span>
          ...
          <div v-if="currentRecording" class="player-ctrl" @click.stop>
            ...
            <div class="ctrl-inner">
              <div class="ctrl-group ctrl-left">
                <button class="ctrl-btn" title="上一段" :disabled="!currentRecording" @click="seekPrev">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="19 20 9 12 19 4"/><line x1="5" y1="4" x2="5" y2="20"/></svg>
                </button>
                <button class="ctrl-btn" title="后退30s" :disabled="!currentRecording" @click="stepBackward">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"/></svg>
                </button>
                <button class="ctrl-btn ctrl-play" :disabled="!currentRecording" @click="togglePlay">
                ...
                <button class="ctrl-btn" title="前进30s" :disabled="!currentRecording" @click="stepForward">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>
                </button>
                <button class="ctrl-btn" title="下一段" :disabled="!currentRecording" @click="seekNext">
              ...
              <div class="ctrl-group ctrl-right">
                <div class="ctrl-speed">
                  <select v-model="playbackSpeed" :disabled="!currentRecording" class="speed-select">
                ...
                <button class="ctrl-btn" title="全屏" :disabled="!currentRecording" @click="toggleFullscreen">
        ...
        <div class="pb-timeline">
          <div class="tl-bar">
            <div class="tl-zoom">
              <button v-for="z in zoomLevels" :key="z.value" class="tl-zbtn" :class="{ active: timelineZoom === z.value }" @click="timelineZoom = z.value">{{ z.label }}</button>
            </div>
            <div class="tl-range">{{ searchStartTime || "00:00" }} — {{ searchEndTime || "24:00" }}</div>
          </div>

          <div ref="timelineRef" class="tl-track-wrap" @click="onTimelineClick">
            <div class="tl-ruler">
              <div v-for="(h, i) in rulerHours" :key="i" class="tl-ruler-cell" :style="{ width: rulerCellWidth + 'px' }">
            ...
            <div class="tl-body">
              <div v-if="hasRecordings" class="tl-track">
                <div
                  ...
                  class="tl-seg"
                  :class="[r.record_type === 'ALARM' ? 'seg-alarm' : 'seg-ok', { 'seg-active': currentRecording?.id === r.id }]"
            ...
            <div v-if="currentRecording" class="tl-playhead" :style="{ left: playheadPercent + '%' }">
...
async function initMpegtsPlayer(url: string) {
  ...
  } catch (e) { console.error("mpegts load failed:", e); }
}
...
function onVideoEnded() { isPlaying.value = false; seekNext(); }
function onVideoError() { loading.value = false; isPlaying.value = false; }
...
async function handleSearch() {
  ...
  destroyMpegtsPlayer();
  currentVideoUrl.value = ""; currentRecording.value = null; isPlaying.value = false;
  try {
    const res = await getRecordFileList({
      page_size: 100, camera_id: selectedCamera.value,
...
watch(playbackSpeed, (val) => { const v = getVideoEl(); if (v) v.playbackRate = val; });
watch(volume, (val) => { const v = getVideoEl(); if (v) v.volume = val / 100; });

onMounted(() => {
  const now = new Date();
  searchDate.value = now.toISOString().slice(0, 10);
  getCameraList({ page_size: 100 }).then(r => { cameras.value = r.data?.data?.items || []; }).catch(() => {});
  getCameraGroupList().then(r => { groupList.value = r.data?.data || []; }).catch(() => {});
});

onBeforeUnmount(() => { destroyMpegtsPlayer(); });
</script>
...
.brand-dot { width: 8px; height: 8px; background: var(--el-color-success); border-radius: 50%; box-shadow: 0 0 6px color-mix(in srgb, var(--el-color-success) 50%, transparent); }
.brand-text { font-size: 15px; font-weight: 600; color: var(--el-text-color-primary); letter-spacing: 0.5px; }

.topbar-sep { width: 1px; height: 20px; background: var(--el-border-color); flex-shrink: 0; }

.layout-btn {
  display: flex; align-items: center; justify-content: center;
  width: 28px; height: 28px; padding: 0;
  color: var(--el-text-color-secondary); cursor: pointer;
  background: var(--el-fill-color); border: 1px solid var(--el-border-color); border-radius: 4px;
  transition: all 0.15s;
}
.layout-btn:hover { color: var(--el-text-color-regular); background: var(--el-fill-color-light); border-color: var(--el-border-color-hover); }
.layout-btn.active { color: var(--el-color-primary); background: color-mix(in srgb, var(--el-color-primary) 10%, transparent); border-color: var(--el-color-primary); }

.topbar-center { flex: 1; display: flex; justify-content: center; }

.search-group { display: flex; align-items: center; }
.field-row { display: flex; align-items: center; gap: 4px; }
.field-row :deep(.el-input__wrapper) { background: transparent !important; box-shadow: none !important; padding: 0 4px; }
.field-row :deep(.el-input__inner) { color: var(--el-text-color-primary) !important; font-size: 12px; height: 28px; }
.field-date { width: 130px; }
.field-time { width: 90px; }
.time-sep { color: var(--el-text-color-placeholder); font-size: 12px; margin: 0 2px; }

.topbar-right { flex-shrink: 0; }
.rec-count-badge {
  display: inline-flex; align-items: center; gap: 4px;
  height: 26px; padding: 0 10px;
  font-size: 12px; font-family: var(--pb-font); color: var(--pb-accent);
...
.pb-icon-btn {
  ...
  color: var(--pb-text2); background: transparent;
  border: 1px solid transparent; border-radius: 5px;
  cursor: pointer; transition: all 0.15s;
}
.pb-icon-btn:hover { color: var(--pb-text); background: var(--pb-border); border-color: var(--pb-border2); }
...
.pb-btn {
  ...
  cursor: pointer; transition: all 0.15s;
  background: var(--pb-surface); color: var(--pb-text2);
}
.pb-btn:hover { border-color: var(--pb-border2); color: var(--pb-text); background: var(--pb-border); }
.pb-btn:disabled { opacity: 0.35; cursor: not-allowed; }
.pb-btn-primary {
  background: var(--pb-accent2); border-color: var(--pb-accent2); color: #fff;
}
.pb-btn-primary:hover { background: var(--pb-accent); border-color: var(--pb-accent); color: #fff; }
.pb-btn-primary:disabled { background: color-mix(in srgb, var(--pb-accent2) 40%, transparent); border-color: transparent; }
...
.tree-filter { flex-shrink: 0; padding: 6px 8px; }
.tree-filter :deep(.el-input__wrapper) { background: var(--el-fill-color); box-shadow: 0 0 0 1px var(--el-border-color) inset; }
.tree-filter :deep(.el-input__inner) { font-size: 12px; color: var(--el-text-color-primary); }
.device-tree { flex: 1; overflow-y: auto; background: transparent; }
.device-tree::-webkit-scrollbar { width: 4px; }
.device-tree::-webkit-scrollbar-thumb { background: var(--el-border-color); border-radius: 2px; }
.device-tree::-webkit-scrollbar-track { background: transparent; }
.device-tree :deep(.el-tree-node__content) { height: 28px; }
.device-tree :deep(.el-tree-node__content:hover) { background: var(--el-fill-color); }
.tree-camera-node { display: flex; align-items: center; gap: 6px; width: 100%; padding-right: 4px; }
.tree-camera-name { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.tree-count { padding: 0 6px; margin-left: auto; font-size: 11px; line-height: 18px; color: var(--el-text-color-placeholder); background: var(--el-fill-color); border-radius: 8px; }
.dot { width: 6px; height: 6px; border-radius: 50%; flex-shrink: 0; }
.dot-on { background: var(--el-color-success); box-shadow: 0 0 6px color-mix(in srgb, var(--el-color-success) 50%, transparent); }
.dot-off { background: var(--el-text-color-placeholder); }

/* ================================================================
   MAIN CONTENT
   ================================================================ */
.pb-content { display: flex; flex-direction: column; flex: 1; overflow: hidden; padding: 0 2px; }
...
.player-video { display: block; flex: 1; width: 100%; object-fit: contain; background: #000; }
.player-flv-wrap { display: flex; flex: 1; width: 100%; background: #000; }
...
.empty-icon svg { color: var(--pb-text3); }
.empty-text { margin: 0; font-size: 13px; color: var(--pb-text3); letter-spacing: 0.3px; }
...
.spinner {
  ...
  border: 2px solid rgba(255,255,255,0.15); border-top-color: var(--pb-accent);
...
.info-cam { font-weight: 600; color: #fff; }
.info-sep { width: 1px; height: 12px; background: rgba(255,255,255,0.15); }
.info-time { font-family: var(--pb-font); font-size: 11px; }
.info-alarm {
  padding: 1px 7px; font-size: 10px; font-weight: 600; color: #fff;
  background: var(--pb-red); border-radius: 3px;
}
...
.ctrl-left { flex-shrink: 0; }
.ctrl-center { flex: 1; justify-content: center; gap: 6px; }
.ctrl-right { flex-shrink: 0; gap: 8px; }
...
.ctrl-btn:disabled:hover { background: transparent; color: rgba(255,255,255,0.3); }

.ctrl-play { width: 36px; height: 36px; color: #fff; background: var(--pb-accent2); border-radius: 50%; }
.ctrl-play:hover { background: var(--pb-accent); }

.ctrl-time {
  font-family: var(--pb-font); font-size: 13px; color: rgba(255,255,255,0.85);
  letter-spacing: 0.5px;
}
.ctrl-dur { color: rgba(255,255,255,0.4); }
.ctrl-time-sep { color: rgba(255,255,255,0.2); font-size: 12px; }

.speed-select {
  height: 26px; padding: 0 6px; font-size: 11px; font-family: var(--pb-font);
  color: rgba(255,255,255,0.7); background: rgba(255,255,255,0.08);
  border: 1px solid rgba(255,255,255,0.1); border-radius: 3px; outline: none;
  cursor: pointer; appearance: none;
}
.speed-select:hover { background: rgba(255,255,255,0.12); }
.speed-select option { background: #222; color: #eee; }

.ctrl-vol { display: flex; align-items: center; gap: 6px; color: rgba(255,255,255,0.5); }
.vol-slider { width: 52px; height: 3px; appearance: none; background: rgba(255,255,255,0.15); border-radius: 2px; outline: none; cursor: pointer; }
.vol-slider::-webkit-slider-thumb { appearance: none; width: 10px; height: 10px; border-radius: 50%; background: #fff; border: none; cursor: pointer; }
.vol-slider::-moz-range-thumb { width: 10px; height: 10px; border-radius: 50%; background: #fff; border: none; cursor: pointer; }
...
.tl-zoom { display: flex; gap: 3px; }
.tl-zbtn {
  padding: 2px 10px; font-size: 11px; font-family: var(--pb-font); color: var(--pb-text3);
  cursor: pointer; background: transparent;
  border: 1px solid transparent; border-radius: 3px; transition: all 0.12s;
}
.tl-zbtn:hover { color: var(--pb-text); border-color: var(--pb-border2); }
.tl-zbtn.active { color: var(--pb-accent); background: color-mix(in srgb, var(--pb-accent) 10%, transparent); border-color: color-mix(in srgb, var(--pb-accent) 30%, transparent); }

.tl-range { font-family: var(--pb-font); font-size: 11px; color: var(--pb-text3); }

.tl-track-wrap {
  position: relative; padding: 6px 14px 22px;
  overflow-x: auto; cursor: pointer; user-select: none;
}
.tl-track-wrap::-webkit-scrollbar { height: 4px; }
.tl-track-wrap::-webkit-scrollbar-thumb { background: var(--pb-border2); border-radius: 2px; }

.tl-ruler { display: flex; height: 24px; margin-bottom: 4px; border-bottom: 1px solid var(--pb-border); }
.tl-ruler-cell { position: relative; flex-shrink: 0; border-left: 1px solid var(--pb-border2); }
.tl-ruler-label {
  position: absolute; top: 4px; left: 4px;
  font-family: var(--pb-font); font-size: 10px; color: var(--pb-text3);
}

.tl-body { position: relative; min-height: 32px; }
...
.tl-seg:hover { opacity: 0.85; z-index: 2; filter: brightness(1.2); }
.tl-seg.seg-ok { background: var(--pb-accent); opacity: 0.6; }
.tl-seg.seg-alarm { background: var(--pb-red); opacity: 0.7; }
.tl-seg.seg-active { opacity: 1; box-shadow: 0 0 0 1.5px #fff; z-index: 3; }

.tl-empty {
  display: flex; gap: 6px; align-items: center; justify-content: center;
  height: 28px; font-size: 11px; color: var(--pb-text3);
  background: var(--pb-bg); border: 1px solid var(--pb-border); border-radius: 4px;
}
.tl-empty-idle { color: var(--pb-text3); opacity: 0.5; }

/* Playhead */
.tl-playhead {
  position: absolute; top: 6px; bottom: 22px;
  z-index: 4; pointer-events: none; transition: left 0.25s ease;
}
.tl-ph-dot {
...
.tl-ph-line {
  position: absolute; top: 34px; bottom: 0; left: 50%;
  width: 2px; background: var(--pb-red);
  transform: translateX(-50%);
}
</style>
