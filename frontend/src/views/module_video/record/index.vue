<template>
  <div class="app-container record-page">
    ...
    <template v-if="activeTab === 'plan'">
        ...
        <template #table="{ data, loading, tableRef, onSelectionChange }">
          <div class="data-table__content">
            ...
            >
              ...
              <el-table-column v-if="contentCols.find((col) => col.prop === 'selection')?.show" type="selection" width="55" align="center" />
              <el-table-column v-if="contentCols.find((col) => col.prop === 'index')?.show" type="index" fixed label="序号" min-width="60" align="center" />
              <el-table-column v-if="contentCols.find((col) => col.prop === 'camera')?.show" key="camera" label="摄像机" min-width="160">
                <template #default="scope">{{ scope.row.camera?.name || `#${scope.row.camera_id}` }}</template>
              </el-table-column>
              <el-table-column v-if="contentCols.find((col) => col.prop === 'plan_type')?.show" key="plan_type" label="计划类型" width="110" align="center">
                <template #default="scope">
                  <el-tag :type="planTypeTag(scope.row.plan_type) as any" size="small" effect="plain">{{ planTypeLabel(scope.row.plan_type) }}</el-tag>
                </template>
              </el-table-column>
              <el-table-column v-if="contentCols.find((col) => col.prop === 'schedule')?.show" key="schedule" label="录制时段" min-width="200">
                <template #default="scope">
                  <span v-if="scope.row.plan_type === 'CONTINUOUS'" class="schedule-summary">全天</span>
                  <span v-else-if="scope.row.plan_type === 'SCHEDULE' && scope.row.schedule_json" class="schedule-summary">{{ scheduleSummary(scope.row.schedule_json) }}</span>
                  <span v-else-if="scope.row.plan_type === 'EVENT'" class="schedule-summary">事件触发</span>
                  <span v-else-if="scope.row.plan_type === 'ALARM'" class="schedule-summary">告警触发</span>
                </template>
              </el-table-column>
              <el-table-column v-if="contentCols.find((col) => col.prop === 'pre_record_sec')?.show" key="pre_record_sec" label="预录" width="70" align="center">
                <template #default="scope">{{ scope.row.pre_record_sec }}s</template>
              </el-table-column>
              <el-table-column v-if="contentCols.find((col) => col.prop === 'post_record_sec')?.show" key="post_record_sec" label="延录" width="70" align="center">
                <template #default="scope">{{ scope.row.post_record_sec }}s</template>
              </el-table-column>
              <el-table-column v-if="contentCols.find((col) => col.prop === 'storage_days')?.show" key="storage_days" label="存储" width="70" align="center">
                <template #default="scope">{{ scope.row.storage_days }}天</template>
              </el-table-column>
              <el-table-column v-if="contentCols.find((col) => col.prop === 'stream_type')?.show" key="stream_type" label="码流" width="70" align="center">
                <template #default="scope"><el-tag size="small">{{ scope.row.stream_type === "MAIN" ? "主" : "子" }}</el-tag></template>
              </el-table-column>
              <el-table-column v-if="contentCols.find((col) => col.prop === 'status')?.show" key="status" label="状态" width="70" align="center">
                <template #default="scope"><el-switch v-model="scope.row.status" disabled /></template>
              </el-table-column>
              <el-table-column v-if="contentCols.find((col) => col.prop === 'created_time')?.show" key="created_time" label="创建时间" prop="created_time" min-width="170" sortable />
              <el-table-column v-if="contentCols.find((col) => col.prop === 'operation')?.show" fixed="right" label="操作" align="center" min-width="150">
                <template #default="scope">
                  <el-button v-hasPerm="['module_video:record:update']" type="primary" size="small" link icon="edit" @click="handleOpenDialog('update', scope.row.id)">编辑</el-button>
                  <el-button v-hasPerm="['module_video:record:delete']" type="danger" size="small" link icon="delete" @click="handleRowDelete(scope.row.id)">删除</el-button>
                </template>
              </el-table-column>
            </el-table>
          </div>
      ...
      <EnhancedDialog v-model="dialogVisible.visible" :title="dialogVisible.title" append-to-body width="720px" @close="handleCloseDialog">
        <el-form ref="dataFormRef" :model="formData" label-width="110px" size="default">
          <div class="form-section">
            <div class="form-section-title">基础信息</div>
            <el-form-item label="摄像机" prop="camera_id">
              <el-select v-model="formData.camera_id" filterable placeholder="请选择摄像机" style="width:300px">
          ...
          <div v-if="formData.plan_type === 'SCHEDULE'" class="form-section">
            <div class="form-section-title">录制时段 <span class="form-section-desc">点击或拖拽选择需要录像的时间段</span></div>
          ...
          <div class="form-section">
            ...
            <template v-if="formData.plan_type === 'EVENT' || formData.plan_type === 'ALARM'">
              <el-form-item label="事件前录制" prop="pre_record_sec">
                <div class="config-field">
                  <el-input-number v-model="formData.pre_record_sec" :min="0" :max="30" :step="1" controls-position="right" style="width:130px" />
              ...
              <el-form-item label="事件后录制" prop="post_record_sec">
                <div class="config-field">
                  <el-input-number v-model="formData.post_record_sec" :min="0" :max="30" :step="1" controls-position="right" style="width:130px" />
                  <span class="config-unit">秒</span>
            ...
            <el-form-item label="录像保存" prop="storage_days">
              <div class="config-field">
                <el-input-number v-model="formData.storage_days" :min="1" :max="365" :step="1" controls-position="right" style="width:130px" />
          ...
          <div class="form-section">
            ...
            <el-form-item label="状态" prop="status"><el-switch v-model="formData.status" /></el-form-item>
            <el-form-item label="备注" prop="description"><el-input v-model="formData.description" type="textarea" :rows="2" placeholder="可选备注" /></el-form-item>
          </div>
        </el-form>
        <template #footer>
    ...
    <template v-if="activeTab === 'log'">
      <div class="log-toolbar">
        <div class="log-filters">
          <el-select v-model="logFilter.camera_id" placeholder="摄像机" clearable filterable style="width:160px" size="small">
            <el-option v-for="c in cameraOptions" :key="c.id" :label="c.name" :value="c.id" />
          </el-select>
          <el-select v-model="logFilter.status" placeholder="状态" clearable style="width:120px" size="small">
            <el-option label="录制中" value="RECORDING" />
            <el-option label="已完成" value="COMPLETED" />
            <el-option label="失败" value="FAILED" />
            <el-option label="已停止" value="STOPPED" />
          </el-select>
          <el-select v-model="logFilter.trigger_type" placeholder="触发方式" clearable style="width:130px" size="small">
            <el-option label="定时" value="SCHEDULED" />
            <el-option label="手动" value="MANUAL" />
            <el-option label="告警" value="ALARM" />
          </el-select>
          <el-date-picker v-model="logFilter.dateRange" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" value-format="YYYY-MM-DD HH:mm:ss" size="small" style="width:240px" />
          <el-button size="small" type="primary" @click="fetchLogs">搜索</el-button>
          <el-button size="small" @click="resetLogFilter">重置</el-button>
        </div>
      </div>
      <div class="log-table-wrap">
        <el-table v-loading="logLoading" :data="logItems" border stripe height="100%" style="width:100%">
          ...
          <el-table-column prop="stream_id" label="流ID" min-width="160">
            <template #default="{ row }"><span class="mono">{{ row.stream_id }}</span></template>
          </el-table-column>
          <el-table-column label="触发方式" width="90" align="center">
            <template #default="{ row }">
              <el-tag :type="triggerTagType(row.trigger_type)" size="small" effect="plain">{{ triggerLabel(row.trigger_type) }}</el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column label="开始时间" prop="start_time" width="170" sortable>
            <template #default="{ row }"><span class="mono">{{ row.start_time }}</span></template>
          </el-table-column>
          <el-table-column label="结束时间" prop="end_time" width="170" sortable>
            <template #default="{ row }"><span class="mono">{{ row.end_time || '-' }}</span></template>
          </el-table-column>
...
import {
  getRecordPlanList, createRecordPlan, updateRecordPlan, deleteRecordPlan, getRecordLogList,
...
const searchConfig = reactive<ISearchConfig>({
  ...
  formItems: [
    { prop: "camera_id", label: "摄像机", type: "select", options: [],
      attrs: { placeholder: "请选择摄像机", clearable: true, filterable: true, style: { width: "200px" } } },
    { prop: "plan_type", label: "计划类型", type: "select",
      options: [
...
const contentConfig = reactive<IContentConfig<any>>({
  ...
  deleteAction: async (ids: string) => {
    await deleteRecordPlan(ids.split(",").map(Number).filter(n => !isNaN(n)));
  },
...
const formData = reactive({
  ...
  pre_record_sec: 5, post_record_sec: 5, storage_days: 30,
  stream_type: "MAIN" as string, status: true, description: undefined as string | undefined,
});
...
async function createPlan(data: any) { await createRecordPlan(data); }
async function deletePlan(ids: number[]) { await deleteRecordPlan(ids); }
function planTypeLabel(type: string) { return ({ CONTINUOUS: "全天录制", SCHEDULE: "定时录制", EVENT: "事件录制", ALARM: "告警录制" } as Record<string, string>)[type] || type; }
function planTypeTag(type: string): any { return ({ CONTINUOUS: "primary", SCHEDULE: "success", EVENT: "warning", ALARM: "danger" } as Record<string, string>)[type] || ""; }
...
function triggerLabel(t: string) { return ({ SCHEDULED: "定时", MANUAL: "手动", ALARM: "告警" } as Record<string, string>)[t] || t; }
function triggerTagType(t: string): any { return ({ SCHEDULED: "success", MANUAL: "primary", ALARM: "danger" } as Record<string, string>)[t] || ""; }
function statusLabel(s: string) { return ({ RECORDING: "录制中", COMPLETED: "已完成", FAILED: "失败", STOPPED: "已停止" } as Record<string, string>)[s] || s; }
function statusTagType(s: string): any { return ({ RECORDING: "warning", COMPLETED: "success", FAILED: "danger", STOPPED: "info" } as Record<string, string>)[s] || ""; }
...
async function fetchLogs() {
  ...
  } catch { logItems.value = []; logTotal.value = 0; }
...
.record-tabs { display: flex; gap: 0; flex-shrink: 0; border-bottom: 1px solid var(--el-border-color-light); }
.rtab {
  padding: 10px 20px; font-size: 14px; font-weight: 500; color: var(--el-text-color-secondary);
...
/* ===== Log Tab ===== */
.log-toolbar { flex-shrink: 0; padding: 8px 12px; border-bottom: 1px solid var(--el-border-color-light); }
.log-filters { display: flex; gap: 8px; align-items: center; flex-wrap: wrap; }
.log-table-wrap { flex: 1; overflow: hidden; }
.log-pagination { flex-shrink: 0; display: flex; justify-content: flex-end; padding: 8px 12px; }
...
.schedule-summary { font-size: 12px; color: var(--el-text-color-secondary); }
.form-section { padding-bottom: 16px; margin-bottom: 20px; border-bottom: 1px solid var(--el-border-color-lighter); }
.form-section:last-child { padding-bottom: 0; margin-bottom: 0; border-bottom: none; }
.form-section-title { display: flex; gap: 8px; align-items: center; margin-bottom: 14px; font-size: 14px; font-weight: 600; color: var(--el-text-color-primary); }
.form-section-desc { font-size: 12px; font-weight: 400; color: var(--el-text-color-placeholder); }
.config-field { display: flex; gap: 8px; align-items: center; width: 100%; }
.config-unit { min-width: 20px; font-size: 13px; color: var(--el-text-color-secondary); }
.config-hint { font-size: 12px; color: var(--el-text-color-placeholder); white-space: nowrap; }
</style>
