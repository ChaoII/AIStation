<template>
  <div class="dashboard-page">
    ...
    <div class="dash-charts" style="display: grid !important; grid-template-columns: 1fr 1fr !important; gap: 12px !important; width: 100% !important;">
      <div style="grid-column: span 1 !important; min-width: 0 !important; box-sizing: border-box !important; width: 100% !important;">
        <el-card shadow="hover" style="width: 100% !important; min-width: 0 !important;">
          <template #header>
...
const cards = computed(() => [
  {
    label: "数据集",
    val: stats.datasetCount,
    icon: Folder,
    color: "#0984e3",
    bg: "#e8f4fd",
    extra: `${stats.annotatedCount} 张已标注`,
    route: "/annotation/dataset",
  },
  {
    label: "标注任务",
    val: stats.taskTotal,
    ...
    color: "#00b894",
    bg: "#e6faf5",
    extra: `${stats.taskCompleted} 已完成`,
    route: "/annotation/task",
  },
  {
    label: "训练任务",
    val: stats.trainTotal,
    icon: Aim,
    color: "#6c5ce7",
    bg: "#efeaff",
    extra: `${stats.trainRunning} 训练中`,
    route: "/train/task",
  },
  {
    label: "摄像头",
    val: stats.cameraTotal,
    icon: Camera,
    color: "#fdcb6e",
    bg: "#fef9e7",
    extra: `${stats.cameraOnline} 在线`,
    route: "/video/camera",
  },
  {
    label: "告警记录",
    val: stats.alarmCount,
    icon: WarningFilled,
    color: "#ff7675",
    bg: "#ffeeed",
    extra: "最近 7 天",
  ...
  {
    label: "在线用户",
    val: stats.onlineUsers,
    icon: UserFilled,
    color: "#74b9ff",
    bg: "#eef6ff",
    extra: `共 ${stats.totalUsers} 用户`,
    route: "/system/user",
  },
]);
...
const datasetBar = computed(() => ({
  ...
  xAxis: {
    ...
    data: datasetImageCounts.value.map((d) => d.name),
  ...
  series: [
    {
      ...
      data: datasetImageCounts.value.map((d) => d.count),
    },
  ],
}));
...
const taskTypeBar = computed(() => ({
  ...
  xAxis: {
    type: "category" as const,
    data: Object.keys(taskTypeCount).length
      ? Object.keys(taskTypeCount).map((k) => typeLbl(k))
      : ["暂无"],
  ...
  series: [
    {
      type: "bar" as const,
      barWidth: "45%",
      itemStyle: { color: "#00b894", borderRadius: [4, 4, 0, 0] },
      data: Object.keys(taskTypeCount).length ? Object.values(taskTypeCount) : [0],
    },
  ],
}));
...
async function loadAllData() {
  ...
  if (annoR.status === "fulfilled") {
    ...
    if (d) {
      ...
      stats.annotatedCount = d.annotated_image_count || 0;
    }
  }
