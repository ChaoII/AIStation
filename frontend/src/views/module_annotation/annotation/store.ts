...
export type ToolName = "select" | "box" | "rotated_box" | "polygon" | "keypoint" | "ocr" | "classification" | "pan" | "zoom"
...
export const useAnnotationStore = defineStore("annotation", {
  ...
  actions: {
    ...
    nextImage() { if (this.hasNext) this.goToImage(this.currentImageIndex + 1) },
    prevImage() { if (this.hasPrev) this.goToImage(this.currentImageIndex - 1) },
    ...
    setTool(tool: ToolName) { this.currentTool = tool },
    setClassId(id: number) { this.currentClassId = id },
    selectAnnotation(id: string | null) { this.selectedAnnotationId = id },
    setZoom(z: number) { this.zoom = Math.max(0.1, Math.min(10, z)) },
