<template>
  <div class="train-detail-page">
    <div class="detail-header">
      ...
      <ElTag :type="(task?.framework === 'ultralytics' ? 'success' : 'primary') as any" size="small" effect="plain">
        {{ task?.framework === "ultralytics" ? "Ultralytics" : "PaddleX" }}
      </ElTag>
      <ElTag :type="statusTag(task?.status || '') as any" size="small" :effect="task?.status === 'running' ? 'dark' : 'plain'">
    ...
    <ElRow :gutter="16">
      <ElCol :xs="24" :md="12">
        <ElCard shadow="never" class="info-card">
          ...
          <ElDescriptions :column="1" size="small" border>
            <ElDescriptionsItem label="数据集 ID">{{ task?.dataset_id }}</ElDescriptionsItem>
            <ElDescriptionsItem label="创建时间">{{ task?.created_time }}</ElDescriptionsItem>
            <ElDescriptionsItem label="Docker 镜像"><code class="docker-tag">{{ task?.docker_image }}</code></ElDescriptionsItem>
            <ElDescriptionsItem label="创建者 ID">{{ task?.created_id }}</ElDescriptionsItem>
            <ElDescriptionsItem label="开始时间">{{ task?.started_at || "—" }}</ElDescriptionsItem>
      ...
      <ElCol :xs="24" :md="12">
        <ElCard shadow="never" class="info-card">
          ...
          <ElDescriptions v-if="task?.hyperparams" :column="1" size="small" border>
            <ElDescriptionsItem v-for="(val, key) in task.hyperparams" :key="String(key)" :label="String(key)">
              <template v-if="typeof val === 'boolean'">
                <ElTag :type="val ? 'success' : 'info'" size="small">{{ val ? "是" : "否" }}</ElTag>
              </template>
              <template v-else>{{ val }}</template>
            </ElDescriptionsItem>
          </ElDescriptions>
          <ElEmpty v-else :image-size="40" description="无超参数" />
        </ElCard>
      </ElCol>
    </ElRow>
    ...
    <ElCard shadow="never" class="section-card">
      ...
      <div class="action-buttons">
        <template v-if="task?.status === 'pending'">
          <ElButton type="primary" size="default" @click="handleStart">开始训练</ElButton>
          <ElButton size="default" @click="handleEditParams">编辑参数</ElButton>
          <ElButton type="danger" size="default" @click="handleDelete">删除</ElButton>
        </template>
        <template v-else-if="task?.status === 'running'">
          <ElButton type="danger" size="default" @click="handleStop">停止训练</ElButton>
        </template>
        <template v-else-if="task?.status === 'success'">
          <ElButton type="primary" size="default" @click="handleViewModel">查看模型</ElButton>
          <ElButton size="default" @click="handleEvaluate">评估模型</ElButton>
          <ElButton size="default" @click="handleRetrain">重新训练</ElButton>
          <ElButton type="danger" size="default" @click="handleDelete">删除</ElButton>
        </template>
        <template v-else-if="task?.status === 'failed'">
          <ElButton type="primary" size="default" @click="handleRetrain">重新训练</ElButton>
    ...
    <ElCard v-if="displayProgress > 0" shadow="never" class="section-card">
    ...
    <ElCard shadow="never" class="section-card">
      ...
      <ElRow :gutter="12">
        <ElCol :xs="12" :sm="6" :md="3"><div class="metric-item"><span class="metric-val">{{ displayMetrics.epoch }}</span><span class="metric-lbl">Epoch</span></div></ElCol>
        <ElCol :xs="12" :sm="6" :md="3"><div class="metric-item"><span class="metric-val">{{ displayMetrics.boxLoss }}</span><span class="metric-lbl">Box Loss</span></div></ElCol>
        <ElCol :xs="12" :sm="6" :md="3"><div class="metric-item"><span class="metric-val">{{ displayMetrics.clsLoss }}</span><span class="metric-lbl">Cls Loss</span></div></ElCol>
        <ElCol :xs="12" :sm="6" :md="3"><div class="metric-item"><span class="metric-val">{{ displayMetrics.dflLoss }}</span><span class="metric-lbl">Dfl Loss</span></div></ElCol>
        <ElCol :xs="12" :sm="6" :md="3"><div class="metric-item"><span class="metric-val metric-green">{{ displayMetrics.precision }}</span><span class="metric-lbl">Precision</span></div></ElCol>
        <ElCol :xs="12" :sm="6" :md="3"><div class="metric-item"><span class="metric-val metric-blue">{{ displayMetrics.recall }}</span><span class="metric-lbl">Recall</span></div></ElCol>
        <ElCol :xs="12" :sm="6" :md="3"><div class="metric-item"><span class="metric-val metric-orange">{{ displayMetrics.map50 }}</span><span class="metric-lbl">mAP@50</span></div></ElCol>
        <ElCol :xs="12" :sm="6" :md="3"><div class="metric-item"><span class="metric-val metric-purple">{{ displayMetrics.map5095 }}</span><span class="metric-lbl">mAP@50:95</span></div></ElCol>
      </ElRow>
    </ElCard>
    ...
    <ElRow :gutter="16">
      <ElCol :xs="24" :md="12">
        <ElCard shadow="never" class="section-card">
          ...
          <BaseChart v-if="displayMetricsLog.length > 0" :option="lossChartOption" height="280px" />
      ...
      <ElCol :xs="24" :md="12">
        <ElCard shadow="never" class="section-card">
          ...
          <BaseChart v-if="displayMetricsLog.length > 0" :option="valChartOption" height="280px" />
          <div v-else class="chart-empty">等待训练数据...</div>
        </ElCard>
      </ElCol>
    </ElRow>

    <ElCard v-if="displayBestMetrics || displayLastMetrics" shadow="never" class="section-card">
      <template #header><span class="card-title">指标对比</span></template>
      <ElTable :data="compareTableData" border size="small" style="width: 100%">
        <ElTableColumn prop="label" label="指标" width="140" />
        <ElTableColumn label="最优 Epoch">
...
defineOptions({ name: "TrainTaskDetail" });
...
const yoloMetrics = reactive({
  epoch: 0, totalEpochs: 0, boxLoss: 0, clsLoss: 0, dflLoss: 0,
  precision: 0, recall: 0, map50: 0, map5095: 0, gpuMem: "", progress: 0,
});
...
const displayBestMetrics = computed<any>(() => liveMetricsLog.value.length ? liveMetricsLog.value.reduce((b: any, m: any) => (m.map50 > (b.map50 || 0) ? m : b), liveMetricsLog.value[0]) : bestMetrics.value);
const displayLastMetrics = computed<any>(() => (liveMetricsLog.value.length ? liveMetricsLog.value[liveMetricsLog.value.length - 1] : lastMetrics.value));
...
const displayMetrics = computed(() => {
  if (task.value?.status === "running" && yoloMetrics.epoch > 0) {
    return {
  ...
  if (/^\s+all\s+/.test(line)) {
    ...
    if (yoloMetrics.epoch > 0) {
      liveMetricsLog.value.push({
        ...
        precision: yoloMetrics.precision, recall: yoloMetrics.recall, map50: yoloMetrics.map50, map5095: yoloMetrics.map5095,
      });
    }
  }
}
...
async function loadTask() {
  ...
  if (!id) return;
  const r = await TrainAPI.detailTask(id);
  ...
  if (id) {
    try {
      ...
      if (r.data?.data?.logs) {
        ...
        logLineCount.value = (cleaned.match(/\n/g) || []).length;
        if (!task.value?.metrics_log) {
...
.progress-text { font-size: 14px; font-weight: 600; color: #409eff; }
.card-title { font-weight: 600; font-size: 14px; color: #303133; }
.docker-tag { background: #f0f2f5; padding: 2px 6px; border-radius: 4px; font-size: 12px; }
.info-card { height: 100%; }
.section-card { margin-bottom: 16px; }
.action-buttons { display: flex; gap: 8px; flex-wrap: wrap; }
.progress-eta { margin-top: 8px; display: flex; align-items: center; gap: 6px; color: #909399; font-size: 13px; }
.pulse-dot { width: 8px; height: 8px; border-radius: 50%; background: #409eff; animation: pulse 1.5s infinite; }
@keyframes pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.3; } }
.metric-item { background: #fff; border: 1px solid #ebeef5; border-radius: 8px; padding: 12px 8px; text-align: center; }
.metric-val { display: block; font-size: 15px; font-weight: 700; font-family: "Cascadia Code", monospace; color: #303133; }
.metric-lbl { display: block; font-size: 11px; color: #909399; margin-top: 4px; }
.metric-green { color: #67c23a; }
.metric-blue { color: #409eff; }
.metric-orange { color: #e6a23c; }
.metric-purple { color: #9b59b6; }
.chart-empty { height: 280px; display: flex; align-items: center; justify-content: center; color: #909399; }
