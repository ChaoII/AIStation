              ...
              <template v-if="ann.type === 'Keypoint'">
                ...
                <g v-for="(kp, i) in ann.keypoints" :key="i">
                  <circle
                    ...
                    :cy="kp.y * ch"
                    r="6"
                    :fill="kpColorByIndex(Number(i), Number(ann.class_id))"
...
import { AnnotationAPI } from "@/api/module_annotation/index";
...
function getCurrentUserName(): string {
  const curUser = useUserStore().info || {};
  const name = (curUser as any).name || (curUser as any).username || (curUser as any).nickname || "";
  if (name) return name;
  try {
...
onMounted(async () => {
  ...
  try {
    ...
    AnnotationAPI.getImages(t.dataset_id, tid, 1, pageSize).then(r => {
      ...
      if (imgs.length > 0) loadImg(imgs[0].id);
      fetchTaskProgress();
