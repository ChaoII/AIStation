  ...
  <div class="multi-camera-grid" :class="gridClass">
    ...
    >
      <template v-if="w.camera">
        ...
        <div class="window-actions">
          ...
          <ElTooltip :content="w.muted ? '取消静音' : '静音'" placement="top">
          ...
          <ElTooltip content="云台控制" placement="top">
            <button class="action-btn" @click.stop="emit('ptz', w.id)">
              <ElIcon><Monitor /></ElIcon>
            </button>
          </ElTooltip>
          <ElTooltip :content="props.recordingWindows?.[w.id] ? '停止录像' : '开始录像'" placement="top">
            <button class="action-btn" :class="{ recording: props.recordingWindows?.[w.id] }" @click.stop="toggleRecord(w)">
              <ElIcon><VideoCamera /></ElIcon>
            </button>
          </ElTooltip>
          <ElTooltip content="查看回放" placement="top">
          ...
          <ElTooltip content="停止" placement="top">
            <button class="action-btn" @click.stop="emit('removeCamera', w.id)">
              <ElIcon><Icon icon="mdi:stop" /></ElIcon>
            </button>
          </ElTooltip>
        </div>
      </template>
      <div v-else class="window-empty drop-zone" :class="{ 'drag-hover': dragOverWindow === w.id }">
        <ElIcon :size="36" class="empty-camera-icon"><VideoCamera /></ElIcon>
      </div>
    </div>
    ...
    <Teleport to="body">
      ...
      >
        ...
        <div class="context-item" @click="contextAction('fullscreen')"><ElIcon><FullScreen /></ElIcon>最大化</div>
        <div class="context-item" @click="contextAction('snapshot')"><ElIcon><Camera /></ElIcon>截图</div>
        <div class="context-item" @click="contextAction('mute')"><ElIcon><Microphone /></ElIcon>{{ contextMenu.muted ? "取消静音" : "静音" }}</div>
        ...
        <div class="context-item danger" @click="contextAction('close')"><ElIcon><Close /></ElIcon>关闭</div>
        <div class="context-item" @click="contextAction('playback')"><ElIcon><VideoPlay /></ElIcon>查看回放</div>
      </div>
    </Teleport>
  </div>
</template>
