...
# B2b 语义分割/人脸关键点/文档/条码：云契约先行声明、Agent 并行接线（见 _IN_FLIGHT_FAMILIES）。
...
_IN_FLIGHT_FAMILIES = {"sem", "face_landmark", "doc", "barcode"}
...
def test_unsupported_scene_list_is_enumerated():
    ...
    故不可落地清单收敛为 7 项，下表为逐项枚举后的权威清单。
    """
    expected = {
        "ACTION_CLS", "ACTION_SKELETON", "HAND_GESTURE",
        "SAM_SEG", "FACE_REC", "STRANGER", "REID_TRACK",
    }
    assert set(unsupported_scene_codes()) == expected
    assert len(unsupported_scene_codes()) == 7
    assert len(SCENES) == 40
    assert len([c for c, s in SCENES.items() if is_edge_implementable(s)]) == 33
