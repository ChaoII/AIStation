...
# B2a 人脸属性/活体/深度：Agent 侧并行接线中，云契约先行声明（见 _IN_FLIGHT_FAMILIES）。
...
_IN_FLIGHT_FAMILIES = {"face_attr", "face_as", "depth"}
...
def test_agent_capability_reported_families_match_contract():
    """跨仓一致：云契约族集合 = Agent 上报族集合（含 B1 obb/iseg、姿态 pose、B2a 三族）。"""
...
def test_unsupported_scene_list_is_enumerated():
    ...
    故不可落地清单收敛为 11 项，下表为逐项枚举后的权威清单。
    """
    expected = {
        "ACTION_CLS", "ACTION_SKELETON", "HAND_GESTURE",
        "SEM_AREA", "SAM_SEG", "FACE_REC", "STRANGER",
        "FACE_LANDMARK", "DOC_TABLE", "BARCODE", "REID_TRACK",
    }
    assert set(unsupported_scene_codes()) == expected
    assert len(unsupported_scene_codes()) == 11
    assert len(SCENES) == 40
    assert len([c for c, s in SCENES.items() if is_edge_implementable(s)]) == 29
