...
def test_agent_capability_reported_families_match_contract():
    """跨仓一致：云契约族集合 = Agent 上报族集合（含 B1 obb/iseg 与姿态切片 pose）。"""
...
def test_agent_capability_cpp_has_no_family_drift():
    ...
    assert actual == _AGENT_REPORTED_FAMILIES, (
        f"Agent 上报族与本测试枚举不一致：AgentOnly={sorted(actual - _AGENT_REPORTED_FAMILIES)}, "
...
def test_unsupported_scene_list_is_enumerated():
    ...
    assert len(unsupported_scene_codes()) == 14
    assert len(SCENES) == 40
    assert len([c for c, s in SCENES.items() if is_edge_implementable(s)]) == 26
