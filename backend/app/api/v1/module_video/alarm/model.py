...
from sqlalchemy import Integer, String, DateTime, Text, Boolean, ForeignKey, JSON
...
class AlarmRuleModel(ModelMixin, UserMixin):
    ...
    algorithm_task_id: Mapped[Optional[int]] = mapped_column(Integer, nullable=True, comment="算法任务ID")
    severity: Mapped[str] = mapped_column(String(16), default="WARNING", comment="严重级别: INFO/WARNING/CRITICAL")

    detect_region: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True, comment="检测区域(多边形坐标)")
    ...
    notify_channels: Mapped[Optional[list]] = mapped_column(JSONB, nullable=True, comment="通知方式: [WS_PUSH, SMS, EMAIL]")
...
class AlarmRecordModel(MappedBase):
    ...
    camera_id: Mapped[int] = mapped_column(Integer, ForeignKey("video_cameras.id", ondelete="CASCADE"), nullable=False, index=True)
    camera: Mapped[Optional["CameraModel"]] = relationship(lazy="selectin")
    rule_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("video_alarm_rules.id", ondelete="SET NULL"), nullable=True)
    ...
    snapshot_path: Mapped[Optional[str]] = mapped_column(String(512), nullable=True, comment="告警截图")
    video_clip_path: Mapped[Optional[str]] = mapped_column(String(512), nullable=True, comment="告警视频片段")

    alarm_time: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True, default=datetime.now, comment="告警时间")
    confirm_time: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True, comment="确认时间")
    confirm_user: Mapped[Optional[str]] = mapped_column(String(64), nullable=True, comment="确认人")

    status: Mapped[str] = mapped_column(String(16), default="PENDING", comment="状态: PENDING/CONFIRMED/IGNORED/FALSE_ALARM")
    ai_result: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True, comment="AI检测原始数据")
